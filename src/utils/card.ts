...
export async function renderQuoteCard(options: RenderQuoteCardOptions): Promise<Buffer> {
  ...
  if (!effects.has('remove-watermark')) drawBrandMark(ctx)
...
function findBestFontSize(
  ...
  for (let fontSize = maxFontSize; fontSize >= minFontSize; fontSize -= 2) {
    ...
    if (lines.length <= maxLines && contentHeight <= area.height) return fontSize
  }
...
function loadRemoteImage(url: string): Promise<LoadedImage> {
  ...
  const request = makeRequest(url, {
    ...
    .catch(error => {
      if (remoteImageCache.get(url) === request) remoteImageCache.delete(url)
  ...
  if (remoteImageCache.size > REMOTE_IMAGE_CACHE_LIMIT) {
    ...
    if (oldestUrl) remoteImageCache.delete(oldestUrl)
  }
