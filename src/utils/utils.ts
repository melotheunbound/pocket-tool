...
export function msToApproxTime(ms: number): string {
  const format = (value: number) => (value % 1 === 0 ? value.toString() : value.toFixed(1))

  if (ms < TIME_UNITS.m) return `~${format(ms / 1000)}s`
  if (ms < TIME_UNITS.h) return `~${Math.round(ms / TIME_UNITS.m)}m`
  if (ms < TIME_UNITS.d) return `~${format(ms / TIME_UNITS.h)}h`
  if (ms < TIME_UNITS.y) return `~${format(ms / TIME_UNITS.d)}d`

  return `~${format(ms / TIME_UNITS.y)}y`
}
...
export function toComponentEmoji(name: keyof typeof Emoji): APIMessageComponentEmoji {
  ...
  if (!match) throw new Error(`Invalid emoji format: ${emoji}`)
...
export function getSubcommandPaths(options: APIApplicationCommandOption[] | undefined, parentPath = ''): string[] {
  if (!options?.length) return []
  ...
  for (const option of options) {
    if (option.type === ApplicationCommandOptionType.Subcommand)
      ...
      paths.push(...getSubcommandPaths(option.options, parentPath ? `${parentPath} ${option.name}` : option.name))
  }

  return paths
}

export const readableSizeUnits = ['kB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB']

export function readableSize(bytes: number, micro: boolean = false, precision = 1): string {
